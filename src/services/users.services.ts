...
class UsersService {
  ...
  private signAccessTokenAndRefeshToken({ user_id, verify }: { user_id: string; verify: UserVerifyStatus }) {
  ...
  async register(payload: RegisterReqBody) {
    ...
    const [access_token, refresh_token] = await this.signAccessTokenAndRefeshToken({
      user_id: user_id.toString(),
      verify: UserVerifyStatus.Unverified
    })
  ...
  async login({ user_id, verify }: { user_id: string; verify: UserVerifyStatus }) {
    const [access_token, refresh_token] = await this.signAccessTokenAndRefeshToken({ user_id, verify })
